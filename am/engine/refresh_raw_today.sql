-- Member-level raw rows for CURRENT_DATE (submitted today). Output cell `rows` = compact JSON array.
-- Schema per row: [date_MMDD, agent, mobile, full_name, bucket_code, outcome_code, in_call(0/1), util_code]
WITH bs AS (
  SELECT ihf.uid form_uid, dmf.account_uid, dmf.mobile, dmf.full_name nm, split_part(ihf.care_reachout__assigned_to,'@',1) ag,
    (ihf.submitted_at::timestamptz)::date sd, to_char((ihf.submitted_at::timestamptz)::date,'MM-DD') d, ihf.care_reachout__outcome outcome,
